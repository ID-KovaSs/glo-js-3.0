...
let shopHireEmployersItem = document.querySelectorAll('.hire-employers-item');
	...
	function shopOpenBtn() {
		shopOpen.textContent = 'Закрыть магазин';
		shopOpen.style.backgroundColor = 'red';
		if(mainList.budget != '') {
			mainList.budget = 0;
		}
		...
		mainInfo.classList.add('fadeInLeft');
		mainFunctions.classList.add('fadeInRight');
		}
	...
	function shopCloseBtn() {
		...
		mainFunctions.classList.add('fadeOutRight');
		setTimeout(locationReload, 900);
	}

	function shopMainHide() {
	...
	shopOpen.addEventListener('click', () => {
		mainList.timeOpen = (mainList.timeOpen  == 'true') ? 'false' : 'true';
		console.log(mainList.timeOpen);
		if(mainList.timeOpen  == 'true') {
			shopOpenBtn();
			shopDiscount.classList.add('pulse');
		} else {
			shopCloseBtn();
			setTimeout(shopMainHide, 1000);
		}
	});
